      ...
      <header className="sticky top-0 z-30 border-b border-foreground/5 bg-background/85 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-3">
          <Link to="/admin" className="flex items-center gap-2">
            ...
            <div>
              <p className="font-display text-base font-semibold leading-tight">Mây Café — Quản trị</p>
          ...
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            ...
            <span>{overviewQuery.data ? `${overviewQuery.data.orderCount} đơn đã thanh toán (30 ngày)` : 'Đang tải...'}</span>
          </div>
        </div>
        <nav className="mx-auto max-w-7xl px-2 pb-2 flex flex-wrap gap-1">
          ...
          <button onClick={async () => { await logout(); navigate('/auth/login'); }} className="ml-auto btn-ghost text-xs">
