...
export async function currentReceipt(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const orders = await OrderModel.find({ tableSessionId, participantId, paymentStatus: 'PAID', status: 'SERVED' }).sort({ createdAt: 1 });
    ...
    res.json({ success: true, data: {
      ...
      orders: orders.map((o) => ({ ...o.toObject(), review: reviews.find((r) => r.orderId.toString() === o.id) ?? null })),
    } });
  } catch (e) { next(e); }
}
